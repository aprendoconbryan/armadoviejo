...
(function () {
	...
	var numCoins = 73,//0= personaje, 3=enemigo 1= terreno1 objetos en la escena
		score = 0,
		...
		DeltaPosX=10.5;//GAA incremento de PosX
		ContaVelocidadX=10
		PosX=2000;//320.75;//GAA Offset del CENTRO Sprite en X
		PosXa=2000;//320.75;
		PosXc=2010;//10;//GAA Offset del CENTRO Se Mueve con el Scrolling
		
		ADeltaPosX=10.5;//GAA incremento de PosX anterior
		...
		eSaltando=0;
		eDeltaPosY=0;
		eGravedad=1;//Aceleracion constante hacia abajo
		eConstVelocidadInicial=5;
		...
		Tiempo=3500;//Lleva un conteo del tiempo transcurrido a modo Timer
		TiempoL1=500;//Lleva un conteo del letrero
	...
	function gameLoop () {
	 ...
	 if(Tiempo>0)
	  ...
	  if(PosXa>16000&& GAMEOVER==0){//Logro escapar de la granja
		TiempoL4=200;
		GAMEOVER=1;
	  }
	  //LETREROS----------/\-----------------------------------------------	
	...
	function sprite (options) {
		...
		var De_1a_2;//Se pasa de validar de un cuadro el primero contra todos lo demas, y luego de los demas al primero.
					...
					while(De_1a_2<2){// Priemro asignamos los vaLores de A1x a D2y y luego al revez
						...
						if(((A2x>A1x && A2x<C1x)||(C2x>A1x && C2x<C1x) ||(A2x<A1x && C2x>C1x)) && ((B2y>B1y && B2y<D1y)||(D2y>B1y && D2y<D1y) ||(B2y<B1y && D2y>D1y)) ){
								if(TipoCoins[cont]==1 && ActivoCoins[cont]==1){//Es un Objeto
									...
									if((B1y<B2y && D1y>B2y) &&(D1y-B2y<45)/*Arriba o cerca del borde*/&& De_1a_2==0){//Colision por arriba mata el salto
										//console.log(OffsetYSalto);
										//console.log(PosY);
										//console.log(Piso);
			...
			if(TipoCoins[IndexObjetoEscena]==9/*IndexObjetoEscena==3*/){//Radar del enemigo
				that.y=e_PosY;
				if((e_PosX-PosX)>0)
					that.x=1000;
				else
			...
			if(TipoCoins[IndexObjetoEscena]==16){//Accesorio del hero
				...
				if(herodireccion==1)//Tomo la direccion de la Bala a la derecha
					frameIndex=0;
				that.x = PosX;// Movimiento transerido solo al personaje
				that.y = PosY;// Movimiento transferido solo al personaje
			}
		...
		that.render = function () {
			...
			if(TipoCoins[IndexObjetoEscena]==14){//dibujando la bala
				if(bulletdireccion==2){ //hacia la izquierda
					...
					bulletDistance+=45;
					that.y-=(bulletVelocidadInicial-Gravedad*bulletTiempoSalto);//incremento vertical parabolico
					bulletTiempoSalto++;
				}
			...
			try{
				  // Draw the animation
				  if(ActivoCoins[IndexObjetoEscena]==1){ // Dibujamos solo si esta activo
	...
	function spawnCoin () {
		...
		ActivoCoins[coinIndex]=1;//Todos los objetos vienen activos
		...
		{
			coins[coinIndex].x+=-0;//Correr el escenario hacia otra parte en el principo, menos al heroe
		}
	...
	function salto1 (e) {
        ...
        LocXA = parseInt(touchobj.clientX); // get x position of touch point relative to left edge of browser
		LocYA = parseInt(touchobj.clientY); // get x position of touch point relative to left edge of browser
	...
	function salto2 (e) {
		...
		if((tapY+10<LocYA)/*&&!(tapX>LocXA+15)&&!(tapX+15<LocXA)*/ ){
			key_S_Press=1;
		}
	...
	function salto3 (e) {
		...
		*/
		...
		if((tapX==LocXA)&&(tapY==LocYA)&&TiempoPresionado<6){
			keyFirePress=1;
		}
		//key_S_Press=0;
		//keyRightPress=0;
	...
	function tap (e) {
		...
		for (i = 0/*Excluimos al heroe*/ /*Excluimos al terreno 1*/; i < coins.length; i += 1) {
